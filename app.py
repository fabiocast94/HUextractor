...
if uploaded_ct and uploaded_rt:
    ...
    for f in all_files:
        try:
            ...
            # controlla che sia immagine CT
            if hasattr(ds, "PixelData"):
                slope = float(getattr(ds, "RescaleSlope", 1.0))
                intercept = float(getattr(ds, "RescaleIntercept", 0.0))
                pixel_array = ds.pixel_array * slope + intercept
                ds.pixel_array = pixel_array
                st.session_state.ct_files.append(f)
        except Exception as e:
            st.warning(f"File {f} non valido come CT, saltato. ({e})")
    
    if not st.session_state.ct_files:
        st.error("Nessuna slice CT valida trovata.")
    ...
    # carica RTSTRUCT solo per leggere i nomi delle ROI
...
if st.button("Esegui analisi") and st.session_state.roi_selected:
    if not st.session_state.ct_files or not st.session_state.rt_path:
        ...
        with st.spinner("Estrazione dati e calcolo HU..."):
            # Leggi volume CT
            ct_slices = []
            for f in st.session_state.ct_files:
                try:
                    ds = pydicom.dcmread(f)
                    slope = float(getattr(ds, "RescaleSlope", 1.0))
                    intercept = float(getattr(ds, "RescaleIntercept", 0.0))
                    ...
                    st.warning(f"File {f} saltato durante il calcolo. ({e})")
            
            if not ct_slices:
                st.error("Nessuna slice CT valida per il calcolo.")
            else:
                ct_slices.sort(key=lambda x: float(getattr(x, "ImagePositionPatient", [0,0,0])[2]))
                ct_volume = np.stack([s.pixel_array for s in ct_slices])
                
                # Carica RTSTRUCT
                rtstruct = RTStructBuilder.create_from(dicom_series_path=st.session_state.tmpdir,
                                                       rt_struct_path=st.session_state.rt_path)
